...
感谢你愿意改进 GeneralAgent。本指南面向修改框架仓库本身的贡献者；如果你只想基于框架创建自己的产品，请先阅读 [开发 Agent App](./DEVELOPING_AGENT_APPS.md)。

GeneralAgent 是 Agent App Framework，而不是固定领域的最终 Agent 产品。一次改动是否应该进入核心，通常比代码具体怎么写更重要。
...
- 在 `general-agent.json` 中声明 package 类型、平台、能力和 runtime tool 依赖。
