...
### 2.2 启动 Desktop Renderer

`AGENTWEAVE_APP_ROOT` 成功解析后，本地 sidecar 会把 discovery 快照和对应的 App prompt 一起保留，并通过 `GET /host/bootstrap` 提供快照。若 discovery 的身份或 capability 集合与当前 prompt 不一致，sidecar 会拒绝启动绑定。没有配置 App 时，接口返回 `404`，不会伪造产品能力。

Electron Main 通过自身持有的私有、带认证 sidecar 传输请求这个固定 API path，只接受主 Renderer 窗口发起的调用，限制响应体大小，并在通过 `window.agentWeave.hostBootstrap.load()` 返回前校验完整 discovery wire contract。Preload bridge 不暴露 Manifest 路径、端点、transport credential、可配置 header 或直接文件访问。Host discovery 与本地传输认证是相互独立的检查，任何一项都不能替代另一项。

Desktop Renderer 始终保留 Chat 与 Settings。可选界面只有在相关声明全部一致时才会开放：

- Memory 同时要求 `memory-management`、`memory-provider`，并且 `memoryPersistence` policy 不是 `disabled`。
- 邮箱账号同时要求 `mail-workflows`、`mail-connector`，以及至少一个已声明 Connector。
- 待处理操作同时要求 `action-center`、`durable-actions`、`approval-engine`，并且外部副作用 policy 不是 `deny`。
- Owner Skills 与 Developer Tools 要求 App 的 `skillManagement` policy 不是 `disabled`，同时继续满足原有的已认证 Host policy 或开发 API 检查。

加载中、格式错误、schema 不支持、接口不可用和非 Desktop bootstrap 都必须 fail closed。直接访问已关闭路由时应返回 Settings，不能短暂渲染受限界面。Renderer 可以提供重试，但不能把失败或过期的 discovery 快照继续当作权限依据。
