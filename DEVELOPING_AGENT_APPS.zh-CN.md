...
Electron Main 使用自身持有的 sidecar base URL 请求这个固定接口，只接受主 Renderer 窗口发起的调用，限制响应体大小，并在通过 `window.agentWeave.hostBootstrap.load()` 返回前校验完整 discovery wire contract。Preload bridge 不暴露 Manifest 路径、可由 Renderer 指定的端点、凭据或直接文件访问。本地传输认证仍是独立的 Host 职责，不能由 discovery 替代。
