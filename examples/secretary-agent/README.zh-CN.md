...
需要连接真实邮箱时，按仓库根目录的 [IMAP/SMTP 配置指南](../../MAIL_CONNECTOR_SETUP.zh-CN.md) 创建账号配置并把密码写入 Credential Vault。示例配置见 `mail-account.example.json`；该文件只包含 opaque secret ID，不包含密码。
