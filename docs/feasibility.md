...
- `src/main/skills.ts`
  - skill inventory 读取、校验和 IPC 边界可作为 dev-only diagnostics 或内部 registry 的参考；不复用为终端用户 skill 管理体验。
...
MVP 里的 skill 不需要等同 Codex 插件系统。开发模式可以先做“本地目录 + manifest”的开发者扩展形态；打包时从已验证目录生成冻结 skill bundle/index，生产 runtime 只读取冻结清单：
...
- Dev mode 扫描 `skills/` 目录。
- 校验 manifest，并输出开发者诊断。
- Packaged mode 读取冻结的 skill bundle/index，不做动态目录扫描。
...
- 权限声明。
- developer-facing skill package distribution / install / update。
...
| 过早复制全部参考项目 | 代码体积膨胀，维护困难 | 只复制 Chat/Sessions UI、turn loop 骨架、gateway adapter；Skills 相关代码仅可借鉴 dev diagnostics 或内部 registry 边界 |
...
- Dev mode skill directory scanning。
- manifest schema validation 和开发者诊断。
- build/package 阶段生成冻结 skill bundle/index。
- packaged runtime 读取冻结 index 并执行 command-based tool。
- 示例 skill fixture 用于开发和测试。
...
- Hermes Desktop:
  ...
  - skill 相关 IPC / inventory 读取思路仅作为 dev-only diagnostics 或内部 registry 参考，不作为终端用户 UI。
- cc-switch:
  - `src-tauri/src/proxy/providers/adapter.rs`
  - `src-tauri/src/proxy/providers/transform_codex_chat.rs`
  - `src-tauri/src/proxy/providers/streaming_codex_chat.rs`
