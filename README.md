...
GeneralAgent 是一个面向开发者的 **Agent App Framework**。它提供可复用的 Agent runtime、模型适配、扩展系统、安全边界和跨平台宿主，让一个新 Agent App 的差异主要来自 Prompt、Skills、Connectors、策略和产品界面，而不是对核心 turn loop 的复制与重写。
...
```bash
git clone https://github.com/dale0525/GeneralAgent.git
cd GeneralAgent
...
GENERAL_AGENT_APP_ROOT=output/research-agent pixi run dev
```
